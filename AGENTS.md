...
`apps/buddy` holds the private Worker service and catalogue tooling; `packages/buddy` holds shared schemas and fixtures. Run `just check` before committing and `just package` before releasing the contract package. The service has no public route. Keep source caches, photos, sessions and credentials out of Git.
